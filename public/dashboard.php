...
$stmt = $pdo->prepare('SELECT * FROM shipments WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?');
$stmt->execute([$_SESSION['user_id'], $perPage, $offset]);
$shipments = $stmt->fetchAll();

// Get total count for pagination
$countStmt = $pdo->prepare('SELECT COUNT(*) FROM shipments WHERE user_id = ?');
